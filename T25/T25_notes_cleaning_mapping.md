# GBS library production and data processing for T25

## DNA plates

21 Plates came from Laura Zamorano. In order to get experimental bugs into the same libraries, we had to reorganize samples across the plates. The original plate layouts can be found in `Tcristinae_LD2023_DNA_ID.xlsx`, the suggested reorganization can be found in `kindergarden_3.0.pdf`, and the layout of the working plates can be found in `REORDERED_Tcristinae_LD2023_DNA_ID.xlsx`.

I am building four libraries. To match Laura's reordering, the libraries will named by color: **Blue, Red, Yellow, and Green**. Taking out all 21 source plates, marking the plates with colors corresponding to the library for which each sample with go. Then, taking 6 ul of DNA from each source plate into the working library plates. Starting with Blue library on 6/9/25.
