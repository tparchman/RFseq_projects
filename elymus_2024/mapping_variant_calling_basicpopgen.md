...
Being executed on ponderosa using tapioca pipeline. Commands in bash script (cleaning_bash_ELEL24.sh), executed as below . This was for two S2 NovaSeq lanes generated in October 2024.

```sh
/working/parchman/ELEL24/
```
...
```sh
$ nohup grep -c "^@" ELEL1_S1_L001_R1_001.fastq > ELEL24_1_number_of_rawreads.txt &>/dev/null &
```
The S2 lane for ELEL1 produced XXXXXX reads.

```sh
$ nohup grep -c "^@" ELEL1_S1_L001_R1_001.fastq > ELEL24_2_number_of_rawreads.txt &>/dev/null &
```
...
$ nohup bash cleaning_bash_ELEL24.sh &>/dev/null &
```

Raw data will stay stored in: /backups/rawdata_to_backup/ELEL24/

Number of reads **after** cleaning:

```sh
$ grep -c "^@" ELEL24_1.clean.fastq > ELEL24_1_number_of_cleanreads.txt
```

```sh
$ grep -c "^@" ELEL24_2.clean.fastq > ELEL24_2_number_of_cleanreads.txt
```



After contam cleaning: reads remain
